...
function getAllQue(req,res){
    if (req.query.search || req.query.filter){
        let allQusetionQuery='select q.id as id, u.name as username,q.title as title FROM questions q LEFT JOIN USERS U ON U.id = q.user_id where q.title Like $1 OR q.tags Like $2;'
        ...
        let allQusetionQuery='select  q.id as id, u.name as username,q.title as title FROM questions q LEFT JOIN USERS U ON U.id = q.user_id;'
        DB.client.query(allQusetionQuery)
        .then((queData)=>{
            res.render('questions',{'queData':queData.rows})
        })
...
function myQue(req,res){
    var QusetionQuery='select u.name as username, q.id as que_id, q.title as title,q.description as que_description,q.tags as tags ,q.code as que_code,q.rank as que_rank from questions q LEFT JOIN USERS U ON U.id = q.user_id where q.user_id = $1;'
    DB.client.query((QusetionQuery),[req.body.id])
    .then((queData)=>{  
...
function addAns(req,res){
    ...
    DB.client.query(addNewQueQuery,[req.body.id,req.params.id,req.body.answer,req.body.description,req.body.code,0,false])
    .then((data)=>{
        return res.send({redirect: "hi"});
    })
} 
...
function addRep(req,res){
    let addNewRepQuery = 'INSERT INTO replies (user_id,answer_id,reply,description,code) VALUES ($1,$2,$3,$4,$5);'
    DB.client.query(addNewRepQuery,[req.body.id,req.params.id,req.body.reply,req.body.description,req.body.code])
    .then((data)=>{
        return res.send({redirect: "hi"});
    })
} 
...
function ans_approved(req,res){
    ...
    if(req.body.id == req.body.q_user_id){
        ...
        DB.client.query(ansApproved,[req.body.ans_id,req.body.is_approved])
        .then(()=>{
