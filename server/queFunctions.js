    ...
    .then((queData)=>{  
        ...
        .then((ansData)=>{ 
            ...
            .then(repdata =>{
                ...
                res.render('question',{'queData':queData.rows,'ansData':ansData.rows,repdata:repdata.rows})
            })
...
function myQue(req,res){
    var QusetionQuery='select u.name as username,q.description as description, q.id as que_id, q.title as title,q.description as que_description,q.tags as tags ,q.code as que_code,q.rank as que_rank from questions q LEFT JOIN USERS U ON U.id = q.user_id where q.user_id = $1;'
...
function ans_approved(req,res){
    ...
    if(req.body.id == req.body.q_user_id){
        ...
        DB.client.query(ansApproved,[req.body.is_approved,req.body.ans_id])
        .then(()=>{
